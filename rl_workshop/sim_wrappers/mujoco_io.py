...
class MujocoWriter:
    ...
    def __init__(self, C: ry.Config, verbose=0, friction="0.8 0.1 0.1", solref="0.002 1."):
        self.verbose = verbose
        ...
        b = ET.SubElement(a, "geom", {"friction": friction, "solref": solref}) #, "solimp": "0.9 0.95 0.001"})
    ...
    def addFrame(self, f: ry.Frame, parent: ET.Element):
        ...
        if "mesh" in spec:
            ...
            if not ((len(col) == 2 or len(col) == 4) and col[-1] < 1):
                ...
                if type == "box":
                    ...
                    geom = ET.SubElement(a, "geom", {"type": "cylinder", "size": self.as_str([size[1], 0.5 * size[0]])})
                elif type == "sphere":
        ...
        # has inertia
        if "mass" in spec:
            if geom is not None and "inertia" not in spec:
                ...
                if "inertia" in spec:
                    I = spec["inertia"]
                    if len(I)==6:
                        spec_diagonal = [I[0], I[3], I[5]] #TODO: preserve off-diagonal terms
                    elif len(I)==3:
                        spec_diagonal = I
                    else:
                        raise Exception('NIY')
                    # RAI reports zero inertia for mass-only virtual joint frames,
                    # but MuJoCo requires positive inertia on every moving body.
                    if min(spec_diagonal) > 0.:
                        diagonal = spec_diagonal
                i.set("diaginertia", self.as_str(diagonal))
                if 'com' in spec:
